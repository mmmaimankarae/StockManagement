...
<?php 
    ...
    $msquery = "SELECT P.ProID, P.ProName, SUM(RO.Qty) AS TotalQty, SUM(RO.Qty * P.PricePerUnit) AS TotalSales
                FROM PRODUCT P JOIN RECEIVE_ORDER RO ON P.ProID = RO.ProID JOIN RECEIVE R ON RO.RecID = R.RecID
                WHERE YEAR(R.Period) = YEAR(CURRENT_DATE) AND MONTH(R.Period) = MONTH(CURRENT_DATE) GROUP BY P.ProID, P.ProName;";
    $msresults = mysqli_query($msconnect, $msquery);
    while ($row = mysqli_fetch_array($msresults)) {
