...
<body class="p-3" style="margin-top: 6%">
  ...
  <table id="dataTable" class="table table-striped table-bordered" style="margin-top: 2%">
    ...
    <tbody>
      ...
      $start = "SELECT R.PayTime AS ReceiptDate, R.RecID AS ReceiptID, SUM(RO.Qty * P.PricePerUnit) AS TotalPrice,
      ...
      while ($row = mysqli_fetch_array($msresults)) {
        $summaryPrice += $row['TotalPrice'];
        echo "<tr>";
        echo "<th scope='row' class='text-center'>" . date("d M Y", strtotime($row['ReceiptDate'])) . "</td>";
        echo "<td class='text-center'>" . $row['ReceiptID'] . "</th>";
      ...
      echo "<tr>";
        echo "<td colspan='3' class='text-center' id='sum'>Total</th>";
        echo "<td class='text-center' id='sum'>" . number_format((double)$summaryPrice, 2) . "</td>";
        echo "<td class='text-center' id='sum'>บาท</td>";
        echo "</tr>";
  ...
  <script>
    $(document).ready(function () {
      $('#exportBtn').click(function () {
        var csvContent = "\uFEFF Day Report\n";
        csvContent += "เลขที่ใบเสร็จ,วันที่ออกใบเสร็จ,ราคาทั้งหมด,ชื่อผู้สั่ง\n";
        $('#dataTable tbody tr').each(function () {
          var rowData = $(this).find('th,td:not([id^="sum"])').map(function () {
            return $(this).text();
          }).get().join(",");
          csvContent += rowData + "\n";
        });
        csvContent +=  "Total,," + <?php echo $summaryPrice; ?> + ",บาท\n";
